    ...
    for r in data:
        ...
        for name in r.dtype.names:
            ...
            try:
                ...
                row[name] = val.decode() if isinstance(val, bytes) else str(val)
        rows.append(row)
    return rows
...
def plot_torque_trajectory(all_raw, out_dir):
    ...
    for mode, cols in all_raw.items():
        ...
        if len(tau) == 0:
            continue
        ax.plot(t, tau, label=mode, color=MODE_COLORS.get(mode),
                linewidth=1.0, alpha=0.85)
    ax.set_xlabel("Time [s]")
...
def plot_summary(metrics_rows, out_dir):
    ...
    for ax_idx, (key, ylabel) in enumerate(keys):
        ...
        bars = ax.bar(modes, vals, color=colors, edgecolor="white", width=0.6)
...
def plot_score(metrics_rows, out_dir):
    ...
    colors = [MODE_COLORS.get(m, "#7F7F7F") for m in modes]
    bars = ax.bar(modes, scores, color=colors, edgecolor="white", width=0.6)
...
def main():
    ...
    print(f"Loaded metrics for {len(metrics_rows)} modes")

    # Load raw data for each mode
    all_raw = {}
    for r in metrics_rows:
        ...
        if raw_path.exists():
            ...
            print(f"  raw: {mode} ({len(all_raw[mode].get('timestamp', []))} steps)")
