...
拇指与食指之间的捏合距离通过 `map_gripper()` 映射到夹爪宽度 `[0, 0.04]` m。
该映射基于 pinch_ratio（拇指尖与食指尖距离 / 手掌宽度），经由阈值缩放和平滑后输出。
当前 webcam demo 已将该值写入 `data.ctrl[7]`（Panda 夹爪执行器）。

未对夹爪做闭环控制或抓取力反馈。

## 6. 相对深度映射（可选原型）

手部靠近/远离摄像头的相对运动可通过 `enable_depth_mapping=True` 启用，
映射到机器人目标位置的 x 方向。

- 深度信号来源：MediaPipe landmarks 中手掌尺寸（wrist–middle MCP 距离）的相对变化。
- 输出受 workspace clamp（`origin_x ± 0.10`）和低通滤波限制。
- 此功能默认关闭，因为：
  - 未做相机标定；
  - 手掌尺寸变化受手部姿态影响，不是稳定深度估计算法；
  - 不能当作真实米制深度使用。
